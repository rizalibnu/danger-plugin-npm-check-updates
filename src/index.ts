...
type OutdatedDependencies = Record < string, {current: string;
  latest: string;
  lock: string} >;

type PackegeJson = Record < string,
any >;
...
const npmCheckUpdates = async(options : Options = {}) => {
  ...
  if (monorepo) {
    ...
    try {
      ...
      const monorepoPackages : Packages[] = monorepoPackagesDir.filter(pkg => {
        try {
  ...
  if (excludePackages) {
    if (typeof excludePackages === 'string') {
      packages = packages.filter(pkg => excludePackages !== pkg
        ?.packageJson
          ?.name);
    } else if (Array.isArray(excludePackages)) {
      packages = packages.filter(pkg => !excludePackages.includes(pkg
        ?.packageJson
          ?.name));
    } else if (excludePackages instanceof RegExp) {
      packages = packages.filter(pkg => !excludePackages.test(pkg
        ?.packageJson
          ?.name));
    }
  }

  for (const {packageJson, packageLock}
  of packages) {
    try {
      ...
      const dependencyLockVersion = (dep : string) => packageLock
        ?.dependencies[dep]
          ?.version || '-';
      let ncuResult : Record < string,
        string > = await ncu.run(_options);
      if (!ignoreLockVersion) {
        ncuResult = objectFilter(ncuResult, (key, value) => value.version.includes(dependencyLockVersion(key)));
      }
      const outdatedDependenciesNames = Object.keys(ncuResult);
      const currentDependencies : Record < string,
        string > = ncu.getCurrentDependencies(packageJson);
      ...
      if (outdatedDependenciesNames.length) {
        warn(`
You have ${outdatedDependenciesNames.length} outdated dependencies in ${packageJson
          ?.name}.\n
<details>
<summary>Show Lists</summary>
<table><thead><tr><th width="100%">Dependency</th><th>Current</th><th>Lock</th><th>Latest</th></tr></thead>${outdatedDependenciesNames.map(dependencyName => {
            const {
              current,
              latest,
              lock} = outdatedDependencies[dependencyName];
            return ` < tr > <td>${dependencyName}</td> < td > $ {
                current
              } < /td><td>${lock}</td > <td>${latest}</td> < /tr>`;
          }).join(' ')}</table > </details>`);
      }
    } catch (err) {
      fail(` npm check updates error in $ {
                packageJson
                  ?.name
              }: ` + err.message);
    }
